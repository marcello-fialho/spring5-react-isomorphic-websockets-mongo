...
public class TodoRepositoryInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        if (!emptyDB) {
            java.util.List<Todo> todos = Arrays.asList(new Todo("1", "Learn JavaScript", false),
                new Todo("2", "Learn React", false),
                new Todo("3", "Learn React Router", false),
                new Todo("4", "Learn Redux", false),
                new Todo("5", "Learn RxJS", false));
            BulkOperations ops = mongoTemplate.bulkOps(BulkOperations.BulkMode.UNORDERED, Todo.class);
            ops.insert(todos);
            ops.execute();
        }
    }
